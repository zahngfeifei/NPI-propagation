...
# 主分析版本：H4_minus_H1 contrast
#
# 适配当前 CSV：传播指标与协变量已在同一个 CSV 中
#
# 主分析：
...
OUT_DIR = r"I:\DYF\NPI-4-code\3.步进分析\ABIDE1_新结果4\GroupStats_H4minusH1_contrast_noFD_noSite"
...
OUT_CONTRAST_TXT = os.path.join(
    OUT_DIR,
    "EC_SEC_GroupStats_ASD_vs_HC_H4_minus_H1_contrast_summary.txt"
)
...
OUT_SINGLE_FDR_LONG_CSV = os.path.join(
    OUT_DIR,
    "EC_SEC_GroupStats_ASD_vs_HC_single_metrics_supplementary_FDR_long.csv"
)

OUT_SINGLE_TXT = os.path.join(
    OUT_DIR,
    "EC_SEC_GroupStats_ASD_vs_HC_single_metrics_supplementary_summary.txt"
)
...
print(f"输入被试数: {n_input}")
print(f"无法解析 sub_id 并删除: {n_bad_id}")
print(f"进入分析前被试数: {len(df)}")
...
with open(OUT_CONTRAST_TXT, "w", encoding="utf-8") as f:
    f.write("EC-SEC Primary H4-H1 Contrast Analysis Summary\n")
    ...
    f.write(
        ...
        "H4 DMN and H1 sensory propagation metrics.\n\n"
    )
    ...
    f.write(f"  Rows after ID QC: {len(df)}\n")
...
with open(OUT_SINGLE_TXT, "w", encoding="utf-8") as f:
    f.write("EC-SEC Supplementary Single-Metric GLM Summary Report\n")
    f.write(f"Generated at: {datetime.now()}\n\n")
    ...
    f.write(
        ...
        "for H1-H4 and exploratory propagation metrics.\n\n"
    )
...
print("\n全部分析完成。")
print("=" * 70)
print("Primary H4-H1 contrast:")
print(f"  {OUT_CONTRAST_CSV}")
print(f"  {OUT_CONTRAST_TXT}")
print("")
print("Supplementary single-metric results:")
