...
MODEL_DIRECTORIES = {
    "unadjusted": SCRIPT_DIRECTORY / "ABIDE1_结果2-不控制系统差异",
    "adjusted": SCRIPT_DIRECTORY / "ABIDE1_结果2-控制系统",
}
...
def style_scatter_axis(axis: plt.Axes) -> None:
    ...
    axis.set_xlabel("Within-subject positive output EC-G1 (G*)", labelpad=8)
...
def format_p_value(p_value: float) -> str:
    """Convert p values to significance symbols for the comparison bracket."""
    if p_value < 0.0001:
        return "****"
    if p_value < 0.001:
        return "***"
...
def create_figure() -> plt.Figure:
    ...
    plot_groups_combined(
        axes[0, 0],
        unadjusted_results,
        "Combined scatter —Level-unadjusted model\n(early_slope_scaled ~ G_star * Group + Age + Sex_bin + FIQ)",
    )
    # b：未控制系统差异模型的斜率估计比较
    plot_slope_summary(
        axes[0, 1],
        unadjusted_results["slopes"],
        "Slope comparison —Level-unadjusted model\n(early_slope_scaled ~ G_star * Group + Age + Sex_bin + FIQ)",
    )
    ...
    plot_groups_combined(
        axes[1, 0],
        adjusted_results,
        "Combined scatter —Level-adjusted model\n(early_slope_scaled ~ G_star * Group + C(Level)\n+ Age + Sex_bin + FIQ)",
        title_y=1.25,
    )
    # d：控制系统差异模型的斜率估计比较
    plot_slope_summary(
        ...
        "Slope comparison —Level-adjusted model\n(early_slope_scaled ~ G_star * Group + C(Level)\n+ Age + Sex_bin + FIQ)",
        title_y=1.25,
    )
