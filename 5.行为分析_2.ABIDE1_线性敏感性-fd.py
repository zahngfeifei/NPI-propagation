...
# 脑指标表。
# 该文件至少需要包含 sub_id 和 BRAIN_PREDICTORS 中的脑指标列，例如 Coupling_index_scaled。
...
# sub-Sub51456, ASD, 55.4, M, CALTECH, 126.0, 0.2953
# 本脚本会按 sub_id 将这些协变量合并到脑指标表。
SUBJECT_INFO_FILE = r"I:\DYF\NPI-4-code\subject_info_for_stats.csv"

# 行为量表 CSV 文件夹。
# 本脚本会扫描该文件夹中的所有 .csv 文件，并自动识别每个 CSV 内可数值化的量表列。
# 如果每个 CSV 是一个单量表文件，则每个 CSV 会产生一个模型；如果某个 CSV 内有多个数值量表列，则每列分别建模。
BEHAVIOR_CSV_DIR = r"I:\DYF\NPI-4-code\5.行为分析\ABIDE1_行为量表\ABIDE1\ADOS"
...
# 本版本从 SUBJECT_INFO_FILE 合并协变量：Group、Age、Sex、Site、FIQ、MeanFD。
# 当前仍按原脚本逻辑做 ASD-only 分析：Group 用于 ASD 筛选，不作为模型协变量。
# 模型协变量为 Age + Sex_for_model + FIQ + MeanFD；不加入 Site fixed effect，因为 Site 已通过 ComBat 控制。
#
# 重要说明：
...
SENSITIVITY_ROOT_DIR = os.path.join(
    BASE_DIR,
    "ABIDE1_优先级4_ASD_非线性Spline_多参数敏感性分析_withFD_noSite_df4to10_HC1"
)

BASE_WITHFD_NOSITE_SPEC = {
    "covariates": ["Age", "Sex_for_model", "FIQ", "MeanFD"],
...
SENSITIVITY_ANALYSIS_SPECS = [
    {
        **BASE_WITHFD_NOSITE_SPEC,
        "analysis_set_name": "S0_main_withFD_noSite_AIC_df4to10",
        "spline_df_candidates": list(range(4, 11)),
        "spline_df_selection_method": "AIC",
        "note": "主分析扩展版：Age + Sex + FIQ + MeanFD；不加入 Site fixed effect，Site 已通过 ComBat 控制；AIC 在 df=4--10 中选择 spline df；df>=4 使用 cubic B-spline degree=3。",
    },
    {
        **BASE_WITHFD_NOSITE_SPEC,
        "analysis_set_name": "S1_withFD_noSite_BIC_df4to10",
        "spline_df_candidates": list(range(4, 11)),
        "spline_df_selection_method": "BIC",
        "note": "敏感性分析：Age + Sex + FIQ + MeanFD；不加入 Site fixed effect，Site 已通过 ComBat 控制；BIC 在 df=4--10 中选择 spline df；df>=4 使用 cubic B-spline degree=3。",
    },
    {
        **BASE_WITHFD_NOSITE_SPEC,
        "analysis_set_name": "S2_withFD_noSite_CVRMSE_df4to10",
        "spline_df_candidates": list(range(4, 11)),
        "spline_df_selection_method": "CV_RMSE",
        "note": "敏感性分析：Age + Sex + FIQ + MeanFD；不加入 Site fixed effect，Site 已通过 ComBat 控制；5-fold CV_RMSE 在 df=4--10 中选择 spline df；df>=4 使用 cubic B-spline degree=3。",
    },
]
...
SENSITIVITY_ANALYSIS_SPECS += [
    {
        **BASE_WITHFD_NOSITE_SPEC,
        "analysis_set_name": f"S{fixed_df - 1}_withFD_noSite_fixed_df{fixed_df}",
        "spline_df_candidates": [fixed_df],
        "spline_df_selection_method": "AIC",
        "note": (
            f"敏感性分析：固定 spline df={fixed_df}；Age + Sex + FIQ + MeanFD；"
            "不加入 Site fixed effect，Site 已通过 ComBat 控制；使用 cubic B-spline degree=3。"
        ),
    }
...
SENSITIVITY_ANALYSIS_SPECS += [
    {
        **BASE_WITHFD_NOSITE_SPEC,
        "analysis_set_name": "S10_withFD_noSite_quadraticSpline_AIC_df4to10",
        "spline_df_candidates": list(range(4, 11)),
        "spline_df_selection_method": "AIC",
        "spline_degree": 2,
        "note": "敏感性分析：使用 quadratic B-spline degree=2；Age + Sex + FIQ + MeanFD；不加入 Site fixed effect，Site 已通过 ComBat 控制；AIC 在 df=4--10 中选择 df。",
    }
]
...
SITE_COL = "Site"  # Site 已通过 ComBat 控制；本脚本不把 Site 放入回归模型。
GROUP_COL = "Group"
ID_COL = "sub_id"
...
def apply_sensitivity_spec(spec):
    ...
    COVARIATES_BASE = list(spec.get("covariates", ["Age", "Sex_for_model", "FIQ", "MeanFD"]))
...
def safe_group_label(x):
    ...
    if s in ["1", "ASD", "AUTISM", "AUTISM SPECTRUM DISORDER"]:
        return "ASD"
    if s in ["2", "HC", "CONTROL", "TD", "TDC", "CONTROL GROUP"]:
        return "HC"
    return np.nan


def sex_to_numeric_for_model(x):
    """将 Sex 编码为模型用二元数值变量：M=1, F=0。"""
    if pd.isna(x):
        return np.nan
    ...
    if s in ["M", "MALE", "1"]:
        return 1.0
    if s in ["F", "FEMALE", "2", "0"]:
        return 0.0

    return np.nan


def load_subject_info_table(path):
    """读取 subject_info_for_stats.csv，并规范化 Group、Sex、Site、Age、FIQ、MeanFD。Site 仅保留为审计字段，不进入模型。"""
    if not os.path.exists(path):
        raise FileNotFoundError(f"Subject info file not found: {path}")
    ...
    if ID_COL not in df.columns:
        raise ValueError(f"Subject info table must contain column: {ID_COL}")
    ...
    if "Sex_for_model" not in df.columns:
        if "Sex_bin" in df.columns:
            ...
            df["Sex_for_model"] = df["Sex"].apply(sex_to_numeric_for_model)

    for col in ["Age", "FIQ", "MeanFD", "Sex_for_model"]:
        if col in df.columns:
            df[col] = pd.to_numeric(df[col], errors="coerce")

    if SITE_COL in df.columns:
        df[SITE_COL] = df[SITE_COL].astype(str).str.strip()
        df.loc[df[SITE_COL].isin(["", "nan", "None", "NA", "N/A"]), SITE_COL] = np.nan

    keep_cols = [
        ID_COL, GROUP_COL, "Age", "Sex", "Sex_for_model", SITE_COL, "FIQ", "MeanFD"
    ]
    keep_cols = [c for c in keep_cols if c in df.columns]

    df = df[keep_cols].drop_duplicates(subset=[ID_COL], keep="first").copy()

    return df


# ============================================================
# 4. 读取脑指标表、协变量表和文件夹中的所有行为 CSV
# ============================================================
...
def load_brain_table(path):
    ...
    # 先保证脑指标表每个被试一行，再合并 subject_info_for_stats.csv。
    df = df.drop_duplicates(subset=[ID_COL], keep="first").copy()

    subject_info_df = load_subject_info_table(SUBJECT_INFO_FILE)

    # 以 subject_info_for_stats.csv 为协变量来源。
    # 如果脑指标表中已有同名协变量，先删除，避免 merge 后产生 _x/_y。
    subject_info_cols = [c for c in subject_info_df.columns if c != ID_COL]
    df = df.drop(columns=[c for c in subject_info_cols if c in df.columns], errors="ignore")
    df = df.merge(subject_info_df, on=ID_COL, how="left")

    # Group 用于 ASD-only 筛选；Site 不作为必需模型列，除非显式开启 Site fixed effect。
    required_cols = [ID_COL] + BRAIN_PREDICTORS + COVARIATES_BASE + [GROUP_COL]
    ...
    if missing:
        raise ValueError(
            "Brain + subject_info table is missing critical columns: " + "; ".join(missing) + "\n"
            f"Please check BRAIN_INPUT_FILE={BRAIN_INPUT_FILE} and SUBJECT_INFO_FILE={SUBJECT_INFO_FILE}."
        )
