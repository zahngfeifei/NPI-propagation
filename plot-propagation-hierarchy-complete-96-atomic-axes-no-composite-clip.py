...
# Each e–h panel receives the same compact brain-map legend.  The placement
# keeps the legend in the gap between its statistical plot and brain views.
BRAIN_MAP_COLOR_BAR_SPECS = (
    ("e", 1546, 448),
    ("f", 333, 948),
    ("g", 914, 948),
    ("h", 1552, 948),
)
BRAIN_MAP_COLOR_BAR_WIDTH_PIXELS = 10
BRAIN_MAP_COLOR_BAR_HEIGHT_PIXELS = 72
BRAIN_MAP_COLOR_BAR_TITLE = "β (ASD–HC)"
BRAIN_MAP_COLOR_BAR_SCALE = "×10⁻⁶"
BRAIN_MAP_COLOR_BAR_TICK_LABELS = ("7.5", "0", "−7.5")
BRAIN_MAP_COLOR_STOPS = (
    (0.0, (33, 102, 172)),
    (0.5, (247, 247, 247)),
    (1.0, (178, 24, 43)),
)
...
def loadEmbeddedRowModule(encodedSource: str, rowNumber: int) -> types.ModuleType:
    ...
    # The source rows were originally authored for ABIDE1.  Keep their layout
    # instructions, but resolve every input from the ABIDE2 analysis outputs.
    sourceText = sourceText.replace("ABIDE1", "ABIDE2").replace("abide1", "abide2")

    if rowNumber in (1, 2, 3):
        sourceText = re.sub(
            r"def locateInputPaths\(analysisDirectory: Path\) -> dict\[str, Path\]:.*?(?=\ndef readInputTables)",
            '''def locateInputPaths(analysisDirectory: Path) -> dict[str, Path]:
    projectRoot = analysisDirectory.parents[2]

    def locateRequiredFile(fileName: str) -> Path:
        candidates = sorted(
            path for path in projectRoot.rglob(fileName) if "ABIDE2" in str(path)
        )
        if not candidates:
            raise FileNotFoundError(f"Required ABIDE2 input was not found: {fileName}")
        return candidates[0]

    atlasCandidates = sorted(
        path for path in projectRoot.rglob("Schaefer2018_400Parcels_7Networks_order.txt")
        if "ABIDE2" in str(path)
    )
    if not atlasCandidates:
        raise FileNotFoundError("ABIDE2 Schaefer-400 Yeo-7 label table was not found.")
    curvesDirectoryCandidates = [
        path for path in projectRoot.rglob("curves_per_subject")
        if "ABIDE2_新结果1" in str(path) and countPropagationArrays(path) > 0
    ]
    if not curvesDirectoryCandidates:
        raise FileNotFoundError("ABIDE2 per-subject propagation curves and V_all arrays were not found.")
    return {
        "metrics": locateRequiredFile("EC_SEC_metrics_all_subjects_combat.csv"),
        "friedman": locateRequiredFile("result3_first_arrival_friedman_test.csv"),
        "contrast": locateRequiredFile("EC_SEC_GroupStats_ASD_vs_HC_H4_minus_H1_contrast.csv"),
        "contrastData": locateRequiredFile("EC_SEC_GroupStats_ASD_vs_HC_H4_minus_H1_contrast_data_used.csv"),
        "supplementary": locateRequiredFile("EC_SEC_GroupStats_ASD_vs_HC_single_metrics_supplementary.csv"),
        "atlasLabels": atlasCandidates[0],
        "curvesDirectory": max(curvesDirectoryCandidates, key=countPropagationArrays),
    }
''',
            sourceText,
            flags=re.S,
        ...
        sourceText = re.sub(
            r"def locateInputPaths\(scriptDirectory: Path\) -> dict\[str, Path\]:.*?(?=\ndef readAndValidateInputs)",
            '''def locateInputPaths(scriptDirectory: Path) -> dict[str, Path]:
    projectRoot = scriptDirectory.parents[2]
    ...
    return {
        "levelStatistics": locateRequiredFile("EC_SEC_GroupStats_ASD_vs_HC_single_metrics_supplementary_FDR_long.csv"),
        "slopeWide": locateRequiredFile("roi-early-slope-combat-raw-wide.csv"),
        "roiDictionary": locateRequiredFile("roi-dictionary-and-system-membership.csv"),
    }
''',
            sourceText,
            flags=re.S,
        ...
        sourceText = re.sub(
            r"\n    if slopeTable\[\"Group\"\]\.value_counts\(\)\.to_dict\(\) != \{.*?\}:\n        raise ValueError\([^\n]*\)",
            "",
            sourceText,
        )
...
def renderEmbeddedRows(scriptDirectory: Path) -> tuple[list[Path], list[Path]]:
    ...
    for rowNumber, encodedSource in enumerate(encodedRowSources, start=1):
        ...
        rowModule.main()
        rowOutputDirectory = scriptDirectory / f"abide2-propagation-hierarchy-row-{rowNumber}"
        rowImagePath = rowOutputDirectory / f"abide2-propagation-hierarchy-row-{rowNumber}.png"
        rowSvgPath = rowOutputDirectory / f"abide2-propagation-hierarchy-row-{rowNumber}.svg"
...
def interpolateBrainMapColor(position: float) -> tuple[int, int, int]:
    """Return one red–blue diverging legend color at a normalized position."""
    for (leftPosition, leftColor), (rightPosition, rightColor) in zip(
        BRAIN_MAP_COLOR_STOPS,
        BRAIN_MAP_COLOR_STOPS[1:],
    ):
        if position <= rightPosition:
            fraction = (position - leftPosition) / (rightPosition - leftPosition)
            return tuple(
                round(leftChannel + fraction * (rightChannel - leftChannel))
                for leftChannel, rightChannel in zip(leftColor, rightColor)
            )
    return BRAIN_MAP_COLOR_STOPS[-1][1]


def drawBrainMapColorBars(
    annotationDrawer: ImageDraw.ImageDraw,
    annotationFont: ImageFont.FreeTypeFont,
) -> None:
    """Draw the e–h blue–white–red brain-map color bars on the final raster."""
    for _, left, top in BRAIN_MAP_COLOR_BAR_SPECS:
        for rowOffset in range(BRAIN_MAP_COLOR_BAR_HEIGHT_PIXELS):
            # Red represents the upper end of the color scale and blue the lower end.
            normalizedPosition = 1.0 - rowOffset / (BRAIN_MAP_COLOR_BAR_HEIGHT_PIXELS - 1)
            annotationDrawer.line(
                (
                    left,
                    top + rowOffset,
                    left + BRAIN_MAP_COLOR_BAR_WIDTH_PIXELS - 1,
                    top + rowOffset,
                ),
                fill=interpolateBrainMapColor(normalizedPosition),
            )
        annotationDrawer.rectangle(
            (
                left,
                top,
                left + BRAIN_MAP_COLOR_BAR_WIDTH_PIXELS - 1,
                top + BRAIN_MAP_COLOR_BAR_HEIGHT_PIXELS - 1,
            ),
            outline="black",
            width=1,
        )
        for tickOffset, tickLabel in zip(
            (0, BRAIN_MAP_COLOR_BAR_HEIGHT_PIXELS // 2, BRAIN_MAP_COLOR_BAR_HEIGHT_PIXELS - 1),
            BRAIN_MAP_COLOR_BAR_TICK_LABELS,
        ):
            tickY = top + tickOffset
            annotationDrawer.line(
                (left - 4, tickY, left, tickY),
                fill="black",
                width=1,
            )
        ...
        annotationDrawer.text(
            (left + BRAIN_MAP_COLOR_BAR_WIDTH_PIXELS / 2, top - 5),
            BRAIN_MAP_COLOR_BAR_SCALE,
            fill="black",
            font=annotationFont,
            anchor="ms",
        )
        annotationDrawer.text(
            (left + BRAIN_MAP_COLOR_BAR_WIDTH_PIXELS / 2, top - 22),
            BRAIN_MAP_COLOR_BAR_TITLE,
            fill="black",
            font=annotationFont,
            anchor="ms",
        )
...
def brainMapColorBarSvgParts() -> list[str]:
    """Return editable SVG elements matching the raster e–h color bars."""
    gradientParts = [
        '<linearGradient id="brain-map-red-blue-gradient" x1="0" y1="1" x2="0" y2="0">'
    ]
    for position, color in BRAIN_MAP_COLOR_STOPS:
        gradientParts.append(
            '<stop offset="{:.0f}%" stop-color="#{:02X}{:02X}{:02X}"/>'.format(
                100.0 * position,
                *color,
            )
        )
    gradientParts.append('</linearGradient>')

    colorBarParts = ['<defs>', *gradientParts, '</defs>']
    for panelName, left, top in BRAIN_MAP_COLOR_BAR_SPECS:
        colorBarParts.append(
            f'<g id="brain-map-color-bar-{panelName}">'
            f'<rect x="{left}" y="{top}" '
            f'width="{BRAIN_MAP_COLOR_BAR_WIDTH_PIXELS}" '
            f'height="{BRAIN_MAP_COLOR_BAR_HEIGHT_PIXELS}" '
            'fill="url(#brain-map-red-blue-gradient)" stroke="black" stroke-width="1"/>'
        )
        for tickOffset, tickLabel in zip(
            (
                0,
                BRAIN_MAP_COLOR_BAR_HEIGHT_PIXELS // 2,
                BRAIN_MAP_COLOR_BAR_HEIGHT_PIXELS - 1,
            ),
            BRAIN_MAP_COLOR_BAR_TICK_LABELS,
        ):
            tickY = top + tickOffset
            colorBarParts.append(
                f'<line x1="{left - 4}" y1="{tickY}" '
                f'x2="{left}" y2="{tickY}" '
                'stroke="black" stroke-width="1"/>'
            )
            colorBarParts.append(
                f'<text x="{left - 6}" y="{tickY}" '
                f'font-family="{FINAL_FONT_FAMILY}" font-size="18" '
                'text-anchor="end" dominant-baseline="middle">'
                f'{tickLabel}</text>'
            )
        colorBarParts.extend(
            (
                f'<text x="{left + BRAIN_MAP_COLOR_BAR_WIDTH_PIXELS / 2}" y="{top - 22}" '
                f'font-family="{FINAL_FONT_FAMILY}" font-size="18" '
                'text-anchor="middle" dominant-baseline="middle">'
                f'{BRAIN_MAP_COLOR_BAR_TITLE}</text>',
                f'<text x="{left + BRAIN_MAP_COLOR_BAR_WIDTH_PIXELS / 2}" y="{top - 5}" '
                f'font-family="{FINAL_FONT_FAMILY}" font-size="18" '
                'text-anchor="middle" dominant-baseline="middle">'
                f'{BRAIN_MAP_COLOR_BAR_SCALE}</text>',
            )
        )
        colorBarParts.append('</g>')
    return colorBarParts


def writeBetaColorBarSvg(outputPath: Path) -> None:
    """Save the e–h ASD-minus-HC beta scale as a standalone editable SVG."""
    canvasWidth = 240
    canvasHeight = 330
    colorBarLeft = 145
    colorBarTop = 75
    ...
    colorBarHeight = 210
    gradientParts = [
        '<linearGradient id="beta-red-blue-gradient" x1="0" y1="1" x2="0" y2="0">'
    ]
    for position, color in BRAIN_MAP_COLOR_STOPS:
        gradientParts.append(
            '<stop offset="{:.0f}%" stop-color="#{:02X}{:02X}{:02X}"/>'.format(
    ...
    svgParts = [
        ...
        (
            f'<svg xmlns="{SVG_NS}" width="{canvasWidth}" height="{canvasHeight}" '
            f'viewBox="0 0 {canvasWidth} {canvasHeight}" version="1.1">'
        ),
        ...
        *gradientParts,
        '</defs>',
        (
            f'<text x="{canvasWidth / 2}" y="30" font-family="{FINAL_FONT_FAMILY}" '
            'font-size="22" text-anchor="middle">β (ASD−HC)</text>'
        ),
        (
            f'<text x="{canvasWidth / 2}" y="55" font-family="{FINAL_FONT_FAMILY}" '
            'font-size="20" text-anchor="middle">×10⁻⁶</text>'
        ),
        (
            f'<rect x="{colorBarLeft}" y="{colorBarTop}" width="{colorBarWidth}" '
            f'height="{colorBarHeight}" fill="url(#beta-red-blue-gradient)" '
            'stroke="black" stroke-width="1"/>'
        ),
    ]
    for tickOffset, tickLabel in zip(
        (0, colorBarHeight // 2, colorBarHeight),
        BRAIN_MAP_COLOR_BAR_TICK_LABELS,
    ):
        tickY = colorBarTop + tickOffset
        svgParts.extend(
            (
                f'<line x1="{colorBarLeft - 7}" y1="{tickY}" '
                f'x2="{colorBarLeft}" y2="{tickY}" stroke="black" stroke-width="1"/>',
                f'<text x="{colorBarLeft - 11}" y="{tickY}" '
                f'font-family="{FINAL_FONT_FAMILY}" font-size="20" '
                f'text-anchor="end" dominant-baseline="middle">{tickLabel}</text>',
            )
        )
    svgParts.append('</svg>')
...
def compareSvgWithPngByPanel(
    ...
    """
    renderedSvgPath = outputDirectory / "abide2-propagation-hierarchy-complete-svg-rendered.png"
    differencePath = outputDirectory / "abide2-propagation-hierarchy-complete-svg-vs-png-difference.png"
    reportPath = outputDirectory / "abide2-propagation-hierarchy-complete-svg-vs-png-report.txt"
...
def main() -> None:
    ...
    outputDirectory = scriptDirectory / "abide2-propagation-hierarchy-complete"
    outputDirectory.mkdir(parents=True, exist_ok=True)
    outputPath = outputDirectory / "abide2-propagation-hierarchy-complete.png"
    editableSvgPath = outputDirectory / "abide2-propagation-hierarchy-complete.svg"
    betaColorBarSvgPath = outputDirectory / "abide2-asd-minus-hc-beta-colorbar.svg"

    # Remove obsolete raster output variants while retaining the editable SVG.
    legacyOutputPaths = (
        outputDirectory / "abide2-propagation-hierarchy-complete-reference-resolution.png",
    )
    ...
    try:
        ...
        writeBetaColorBarSvg(betaColorBarSvgPath)
        ...
        for rowNumber in range(1, 5):
            rowOutputDirectory = (
                scriptDirectory / f"abide2-propagation-hierarchy-row-{rowNumber}"
            )
    ...
    print(f"Saved standalone beta color bar to: {betaColorBarSvgPath}")
