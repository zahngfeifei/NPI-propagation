...
# 1. Path settings
# ============================================================
# 当前负向模板对齐脚本输出目录：每个 sub-* 文件夹内为
# out_G1_procrustes.npy / out_G2_procrustes.npy
# ABIDE1 使用独立目录 ABIDE1_负向结果2 时，仅需调整此路径。
INPUT_ROOT = Path(r"I:\DYF\NPI-4-code\2.梯度分析\负向链接-独立模板\ABIDE2_负向结果2")

# ComBat 后输出目录。下游统计脚本可直接把 BASE_DIR/G_ROOT 指向这里。
OUTPUT_ROOT = Path(r"I:\DYF\NPI-4-code\2.梯度分析\负向链接-独立模板\ABIDE2_负向结果2_combat")

# 协变量 CSV：需要包含 sub_id, Group, Age, Sex, Site, FIQ；MeanFD 可存在但本脚本不使用。
DEMO_CSV = Path(r"I:\DYF\NPI-4-code\subject_info_for_stats.csv")
...
# 2. ComBat settings
# ============================================================
# 适配当前输出命名：负向模板对齐脚本保存的是 out_G*_procrustes.npy，
# 不是 FC_G*_procrustes.npy。
GRADIENT_FILES = {
...
# ComBat 中保护诊断组别和性别；连续协变量只使用 Age/FIQ，不使用 MeanFD。
...
def load_covariates(csv_file):
    ...
    # 只保留 CONTINUOUS_COLS 中存在的列，避免误把 MeanFD 放入 ComBat。
    available_continuous_cols = [
        col for col in available_continuous_cols if col in CONTINUOUS_COLS
    ]
    ...
    if before_count != after_count:
        print(f"Dropped {before_count - after_count} rows with missing ComBat covariates.")
...
def find_subject_folders(input_root):
    return sorted([p for p in input_root.glob("sub-*") if p.is_dir()])
...
def match_subjects(subject_folders, covariate_table):
    ...
    for folder in subject_folders:
        ...
        missing_files = [str(path) for path in gradient_paths.values() if not path.exists()]
        if missing_files:
        ...
        for grad_name, path in gradient_paths.items():
            output_file = output_dir / GRADIENT_FILES[grad_name]
            record[f"{grad_name}_input_file"] = path
            record[f"{grad_name}_output_file"] = output_file
            record[f"{grad_name}_output_alias_file"] = output_dir / output_file.name.replace(
                ".npy", "_combat.npy"
            )
    ...
    for record in records:
        try:
            ...
            for grad_name in GRADIENT_FILES.keys():
                loaded[grad_name] = load_gradient_vector(record[f"{grad_name}_input_file"])
...
def save_reports(records, covariates, continuous_columns, feature_reports, valid_feature_masks):
    ...
    with open(REPORT_TXT, "w", encoding="utf-8") as report_file:
        report_file.write("ABIDE1 negative-connectivity sensitivity: gradient vector ComBat report\n")
        report_file.write("==============================================================\n\n")
        ...
        report_file.write("  MeanFD: not included in ComBat covariates\n")
        report_file.write(f"  EXPECTED_FEATURES: {EXPECTED_FEATURES}\n")
        report_file.write(f"  EPS_VARIANCE: {EPS_VARIANCE}\n\n")

        report_file.write("Input gradient files\n")
        for grad_name, file_name in GRADIENT_FILES.items():
            report_file.write(f"  {grad_name}: {file_name}\n")
        report_file.write("\n")
...
# ============================================================
def main():
    print("========== ABIDE1 negative-connectivity gradient vector ComBat ==========")
