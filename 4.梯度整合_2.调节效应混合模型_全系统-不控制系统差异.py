...
4. 模型中不控制 Site，不控制 MeanFD；
5. 在同一个模型中计算 HC、ASD、ASD-HC 的 simple slopes；
6. 输出模型结果、参数表、HC/ASD simple slopes、预测线数据、个体斜率数据；
7. 不绘图。
"""
...
OUT_DIR = Path(
    r"I:\DYF\NPI-4-code\4.梯度整合\ABIDE1_结果2-不控制系统差异"
)
...
def slope_result_to_sentence(row):
    ...
    return (
        f"{group}: slope = {beta:.10g}, p = {p:.10g}, "
        f"{direction}, {sig}"
    )
...
# =========================================================
# 7) 保存模型 summary，并追加 simple slope 结果
# =========================================================
with open(MODEL_SUMMARY_PATH, "w", encoding="utf-8") as f:
    f.write("Result1 all-systems model\n")
    f.write("Model used: cluster-robust OLS\n")
    f.write("System fixed effects: not included in the primary model\n")
    ...
    f.write(
        ...
        "gradient-propagation association\n"
    )
    f.write("\n")
    ...
    for _, row in simple_slopes.iterrows():
        ...
        f.write(f"  slope_beta:    {row['slope_beta']:.10g}\n")
        f.write(f"  SE:            {row['SE']:.10g}\n")
        f.write(f"  z_or_t:        {row['z_or_t']:.10g}\n")
        f.write(f"  p_value:       {row['p_value']:.10g}\n")
        f.write(f"  95% CI:        [{row['ci_low']:.10g}, {row['ci_high']:.10g}]\n")
    ...
    f.write(
        ...
        "all-systems gradient-propagation coupling.\n"
    )
    f.write(
        "Use the ASD row to determine whether ASD shows significant "
        "all-systems gradient-propagation coupling in the same model.\n"
    )
    f.write(
        "Use the ASD_minus_HC row to determine whether ASD differs "
        "significantly from HC in coupling slope.\n"
    )
    f.write("\n")
...
slope_lines.append("Main model simple slope tests:")
slope_lines.append(
    f"  HC slope p-value: {hc_stats['p_value']:.12g}"
)
slope_lines.append(
    f"  ASD slope p-value: {asd_stats['p_value']:.12g}"
)
slope_lines.append(
    f"  ASD-HC interaction term: {interaction_term}"
)
slope_lines.append(
    f"  ASD-HC interaction p-value: {model.pvalues[interaction_term]:.12g}"
)
...
lines.append("System fixed effects: not included in the primary model")
lines.append("Site covariate: not included")
lines.append("MeanFD covariate: not included")
...
lines.append("  HC row: whether HC has significant all-systems coupling.")
lines.append("  ASD row: whether ASD has significant all-systems coupling.")
lines.append("  ASD_minus_HC row: whether ASD-HC coupling slopes differ.")
lines.append("")
