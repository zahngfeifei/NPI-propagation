...
OUT_ROOT = Path(
    r"I:\DYF\NPI-4-code\2.梯度分析\正向连接-参数敏感性分析\ABIDE1_结果6_sensitivity_G1_Default_dist_VisSomMot_only"
)
...
FORMULA = (
    ...
    "C(Group, Treatment(reference='HC')) + Age + FIQ + C(Sex) + MeanFD"
)
...
def load_gradient_vector(
    ...
    if expected_features is not None and len(arr) != expected_features:
        raise ValueError(
            f"梯度长度错误: {path}, "
            f"length={len(arr)}, expected={expected_features}"
        )
...
need = {"sub_id", "Group", "Age", "Sex", "FIQ", "MeanFD"}
...
def run_one_combination(p: float, k_align: int) -> dict:
    ...
    # 性能优化：
    # itertuples 比 iterrows 更快，同时不改变分析内容
    ...
    if "HC" not in df["Group"].cat.categories:
        raise ValueError(
            f"{ptag}/{ktag}: "
            "Group 中未找到 HC，无法作为 baseline。"
        )

    # 保存该组合下的距离指标
    metric_csv = (
        out_dir
        / "G1_Default_dist_VisSomMot_subject_values.csv"
    )
    ...
    result_csv = (
        out_dir
        / "GLM_G1_Default_dist_VisSomMot_main_effect.csv"
    )
    ...
    with open(
        ...
        f.write(
            ...
            "G1_Default_dist_VisSomMot only\n"
        )
        f.write(
            "====================================================\n\n"
        )
        ...
        f.write(
            "  G1_Default_dist_VisSomMot = "
            "|mean_zG1(Default) - mean_zG1(Vis + SomMot)|\n"
        )
        f.write(
            "  This is an unsigned distance metric.\n\n"
        )
        ...
        f.write("Sample size:\n")
        f.write(
            "  info subjects after covariate QC: "
            f"{len(info)}\n"
        )
        f.write(
            "  metric subjects before model NA drop: "
            f"{len(rows)}\n"
        )
        f.write(
            f"  model subjects: {len(df)}\n"
        )
        f.write(
            "  failed or missing subjects: "
            f"{len(failed_subs)}\n\n"
        )
        ...
        f.write("Outputs:\n")
        f.write(
            f"  subject metric CSV: {metric_csv}\n"
        )
        f.write(
            f"  GLM result CSV: {result_csv}\n"
        )
...
overall_csv = (
    OUT_ROOT
    / "overall_G1_Default_dist_VisSomMot_sensitivity_summary.csv"
)
...
overall_txt = (
    OUT_ROOT
    / "overall_G1_Default_dist_VisSomMot_sensitivity_report.txt"
)
...
with open(
    ...
    f.write(
        "  Only one planned metric is tested "
        "in each sensitivity condition.\n\n"
    )
